...
func createFakeTaskService(c *config.Config) (*CollectorService, *gin.Engine) {
	db := connect()
...
func TestTaskFlow(t *testing.T) {
	...
	server.POST("/issues", func(context *gin.Context) {
		state := new(repositories)
		if err := context.BindJSON(state); err != nil {
			runtimeinfo.LogError("(RESP. TO: -> GITHUB-COLLECTOR) JSON UNMARSHAL COMPLETED WITH ERROR: ", err)
			context.AbortWithStatus(http.StatusLocked)
		...
		split := strings.Split(state.Repositories[0], "/")
		name := split[len(split)-1]
		repo, err := service.repository.GetRepositoryByName(name)
		if err != nil {
			runtimeinfo.LogError(err)
			context.AbortWithStatus(http.StatusLocked)
			return
